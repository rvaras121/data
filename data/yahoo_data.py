...
def get_all_calls(ticker: str) -> pd.DataFrame:
    ...
    dates = [dt.datetime.strptime(da, c.YAHOO_DATE_FMT) for da in dates]
    ...
    calls_df['Implied Volatility'] = calls_df['Implied Volatility'].str.replace('%', '').astype(float).div(100)
    return calls_df
